...
void writePanels()
{
    ...
    /*     panFdata(); */
    /*     if (ch_toggle > 3) */
    /*         check_panel_switch(0); */
    /*     if (panel != fdata_prev_panel) */
    /*         osd_statf |= FDATA_OFF_F; */
    /* } else { */
    ...
    if (panel != npanels) {
        ...
        if (ISd(panel, Warn_BIT))
            panWarn(panWarn_XY[0][panel], panWarn_XY[1][panel]);
        ...
        if (ISe(panel, CAM_POS_BIT))
            panCamPos(panCameraPos_XY[0][panel], panCameraPos_XY[1][panel]);
        if (ISd(0, CALLSIGN_BIT))
            panCALLSIGN(panCALLSIGN_XY[0][panel], panCALLSIGN_XY[1][panel]);
        if (get_item_config(EEP_BATT_B_VOLT, panel, &x, &y, &f))
            showBatteryBVolt(x, y, f);
        if (get_item_config(EEP_EF_CLIMB, panel, &x, &y, &f))
            showClimbEfficiency(x, y, f);
        ...
        for (p = 0; p < npanels; p++)
            if (ISd(p, Warn_BIT)) {
                if (ISd(0, Warn_BIT))
                    panWarn(panWarn_XY[0][p], panWarn_XY[1][p]);
                break;
            }
        if (ISd(0, CALLSIGN_BIT))
            panCALLSIGN(panCALLSIGN_XY[0][0], panCALLSIGN_XY[1][0]);        //call sign even in off panel
    }
    //}
    timers();
...
//    osd.printf("%c%3i%c%02i|%c%5.0f%c|%c%5.0f%c|%c%5.0f%c|%c%5.0f%c|%c%5.0f%c|%c%5.0f%c", 0x08,((int)start_Time/60)%60,0x3A,(int)start_Time%60, 0x0b, ((max_home_distance) * converth), high, 0x1b, ((tdistance) * converth), high, 0x13,(max_osd_airspeed * converts), spe,0x14,(max_osd_groundspeed * converts),spe,0x12, (max_osd_home_alt * converth), high,0x1d,(max_osd_windspeed * converts),spe);
    osd.printf_P(PSTR("%c%3i%c%02i|%c%5.0f%c|%c%5.0f%c|%c%5i%c|%c%5i%c|%c%5i%c|%c%5i%c|%c%5.0f%c|%c%11.6f|%c%11.6f"), 0x08, ((int) (start_Time / 60)), 0x3a, (int) start_Time % 60, 0x0b,
                 ((max_home_distance) * converth), high, 0x8f, (tdistance * converth), high, 0x13, (int) (max_osd_airspeed * converts), spe, 0x14, (int) (max_osd_groundspeed * converts), spe, 0x12,
                 (int) (max_osd_home_alt * converth), high, 0x1d, (int) (max_osd_windspeed * converts), spe, 0x17, mah_used, 0x01, 0x03, (double) osd_lat, 0x04, (double) osd_lon);
    check_rssi();
    check_warn();
    panWarn(11, 1);
}
...
//    osd.printf("%c%5.1f%c", 0x0a, (float(temperature * tempconv + tempconvAdd) / 100), temps);
    osd.printf_P(PSTR("%5.1f%c"), temperature, temps);
}
...
void panRSSI(int first_col, int first_line)
{
    ...
    if ((rssiraw_on & 1) == 0)
        ...
        osd.printf_P(PSTR("%c%4i"), 0x09, rssi);
}
...
void check_warn()
{
    ...
    if (osd_vbat_A < float (battv) / 10.0 || (osd_battery_remaining_A < batt_warn_level && batt_warn_level != 0))
        wmask |= 8;
    if (rssi < rssi_warn_level && rssi != -99)
        wmask |= 16;
    if (osd_statf & WARN_MOTOR_F)
        wmask |= 32;
    else if (motor_warn)
        osd_statf |= WARN_MOTOR_F;      // wait for 1 sec for ESC to react before displaying the alert
    if (millis() > last_mav_data_ts + 2200)
        wmask |= 64;
