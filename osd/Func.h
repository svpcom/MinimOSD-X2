...
const PROGMEM char compass_pattern[] =
    { '\x8b', '\x89', '\x8a', '\x89', '\x8a', '\x89', '\x8d', '\x89', '\x8a', '\x89', '\x8a', '\x89', '\x8c', '\x89', '\x8a', '\x89', '\x8a', '\x89', '\x8e', '\x89', '\x8a', '\x89', '\x8a', '\x89'
};
...
//  }
    if (osd_groundspeed > 1.0)
        tdistance += (osd_groundspeed * (millis() - runt) / 1000.0);
    mah_used += (osd_curr_A * 10.0 * (millis() - runt) / 3600000.0);
    runt = millis();
    if (takeofftime == 1) {
        if (osd_home_distance > max_home_distance)
            max_home_distance = osd_home_distance;
        if (osd_airspeed > max_osd_airspeed)
            max_osd_airspeed = osd_airspeed;
        if (osd_groundspeed > max_osd_groundspeed)
            max_osd_groundspeed = osd_groundspeed;
        if (osd_alt_to_home > max_osd_home_alt)
            max_osd_home_alt = osd_alt_to_home;
        if (osd_windspeed > max_osd_windspeed)
            max_osd_windspeed = osd_windspeed;
    }
}

void checkModellType()
{
    if (EEPROM.read(MODELL_TYPE_ADD) != 0)
        EEPROM.write(MODELL_TYPE_ADD, 0);
    if (EEPROM.read(FW_VERSION1_ADDR) != 0)
        EEPROM.write(FW_VERSION1_ADDR, 0);
    if (EEPROM.read(FW_VERSION2_ADDR) != 5)
        EEPROM.write(FW_VERSION2_ADDR, 5);
    if (EEPROM.read(FW_VERSION3_ADDR) != 2)
        EEPROM.write(FW_VERSION3_ADDR, 2);
}
